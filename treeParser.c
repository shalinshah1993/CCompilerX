...
void ex(FILE *xmlSpeciesFile, FILE *xmlReacFile, nodeType *root)
{
	...
	if(!toInitialize){
		...
		fprintf(xmlSpeciesFile, "<!-- This file is generated by Chemical CompilerX -->\n");
					...
					switch(root->opr.oper) 
					{
						...
						case '-':   
							...
							if(!gdsl_stack_is_empty(stack))
							{
								...
								/* Add start to species part. This is required for copy and clear reactions */
								char* idBuffer = (char *) malloc(INTEGER_BUFFER_SIZE);
    							snprintf(idBuffer, INTEGER_BUFFER_SIZE - 1,"%d", REAC_ID);
								fprintf(xmlSpeciesFile, "%s", genSpeciesCode(appendString("start", idBuffer), START_AMOUNT));
								snprintf(idBuffer, INTEGER_BUFFER_SIZE - 1,"%d", REAC_ID + 5 + 1);
								fprintf(xmlSpeciesFile, "%s", genSpeciesCode(appendString("start", idBuffer), START_AMOUNT));
								snprintf(idBuffer, INTEGER_BUFFER_SIZE - 1,"%d", REAC_ID + 5 + 5 + 1);
								fprintf(xmlSpeciesFile, "%s", genSpeciesCode(appendString("start", idBuffer), START_AMOUNT));
								//Occupy the register so change state
								_localz.isEmpty = 0;
								...
								if(lOperand->type == typeCon && rOperand->type == typeCon)
								{
									...
									fprintf(xmlReacFile, "%s", genSubReac (decimalToWords(lOperand->con.value), decimalToWords(rOperand->con.value), REAC_ID));
									REAC_ID = REAC_ID + 15;
								...
								else if(lOperand->type == typeId && rOperand->type == typeId)
								{
									...
									fprintf(xmlReacFile, "%s", genSubReac ((char *)lOperand->id->name, (char *)rOperand->id->name, REAC_ID));
									REAC_ID = REAC_ID + 15;
								...
								else if(lOperand->type == typeCon && rOperand->type == typeId)
								{
									...
									fprintf(xmlReacFile, "%s", genCopyReac (decimalToWords(lOperand->con.value), (char *)rOperand->id->name, REAC_ID));
									REAC_ID = REAC_ID + 15;
								...
								else if(lOperand->type == typeId && rOperand->type == typeCon)
								{
									...
									fprintf(xmlReacFile, "%s", genCopyReac ((char *)lOperand->id->name, decimalToWords(rOperand->con.value), REAC_ID));
									REAC_ID = REAC_ID + 15;
								...
								if(lOperand->type == typeCon && lOperand->con.value > 0)
								{
									printf("Error! Negative values not supported. \n");
									exit(0);									
								}

								if(lOperand->type == typeId && lOperand->id->value > 0)
								{
									printf("Error! Negative values not supported. \n");
									exit(0);
								}
